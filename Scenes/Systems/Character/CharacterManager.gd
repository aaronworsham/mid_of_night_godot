...
var _character_characteristics:CharacteristicsCollectionResource = CharacteristicsCollectionResource.new()
var _dice_manager:DiceManager = DiceManager.new()

func _ready() -> void:
	_character_resource.on_load()
	_character_characteristics.json = characteristics_json
	_character_characteristics.on_load()
...
func set_skill_value(skill_enum:String, v:int) ->void:
	var _skill_resource:SkillMemberResource = _character_skills.get_member_by_enum(skill_enum)   
