...
func before_each():
	...
	timeline_name = dialog_resource._dialogs_array[0]["timeline_name"]
...
func test_get_timelines():
	var tmp:Array = dialog_resource.get_timelines()
	assert_eq(tmp[0], timeline_name)
